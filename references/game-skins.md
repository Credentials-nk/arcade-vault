...
| `frogger`       | FROGGER       | `clasico` | Frogger depende de que el jugador distinga **4 tipos de entidad simultáneos en pantalla** (coche/camión/tronco/tortuga, vía `skin.pieces[0..3]`) bajo presión de tiempo — una decisión de vida o muerte por segundo. El monocromo `retro` reduciría esa distinción a solo brillo/forma (las 8 `pieces` de esa paleta son variaciones de un único ámbar), arriesgando confusiones carretera/río justo donde el juego menos lo perdona. `clasico` da 4 matices claramente diferenciados (teal/mostaza/violeta/verde apagados) sin el glow saturado de neon — mismo criterio de "legibilidad ante todo" ya aplicado a Caída, que enfrenta el mismo problema de distinguir piezas por color a simple vista. |
...
- **`frogger`** — `lib/games/frogger/game.ts` recibe `skin: Skin`; usa `skin.bg`
  (fondo), `skin.primary` (rana + vidas del HUD interno, además de la base para
  el contorno semitransparente de tortugas sumergidas vía `rgbTriplet`),
  `skin.accent` (relleno/borde de las bocas destino y la barra de tiempo en
  nivel medio), `skin.line` y `skin.highlight` (bandas de zona: río / metas /
  franja segura) y `skin.text` (HUD interno de score/nivel). Las 8 `skin.pieces`
  cubren los 4 tipos de entidad simultáneos: `pieces[0]` coches, `pieces[1]`
  camiones, `pieces[2]` troncos, `pieces[3]` tortugas visibles. Sin literales de
  color hardcodeados en el engine.
