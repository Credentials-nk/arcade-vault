...
- **.claude/agents/** — `game-planner`, `game-jam`, `skin-designer`, `mobile-porter`, `game-performance-booster`, `security-auditor` (see "Subagents" below)
...
Production also needs configuration that isn't code (OAuth providers, redirect URLs, password policy, rate limits). That checklist lives in `references/Security/prod-setup.md`.
...
4. **`/spec-impl-game`** — specialization of `/spec-impl` for game specs (the ones `/add-game` or `@game-jam` produce): runs the exact same four phases, then, only on success, chains `@skin-designer` → `@game-performance-booster` → PR → `@mobile-porter` **in sequence**, scoped to just the new game. The skin **and performance** commits land in the same PR as the game; `@mobile-porter`'s touch-layout diff is left uncommitted for real-device validation.
...
- **Use `/spec-impl-game` instead of `/spec-impl` for game specs** — same four phases, plus an automatic `@skin-designer` → `@game-performance-booster` → PR → `@mobile-porter` chain scoped to the new game.
...
- **`@mobile-porter`** — audits and **aligns the mobile/touch layout** of the 4 games so they look and play consistently on phone browsers (`pointer: coarse`). Core principle: the touch layout is a **single immutable shell** (display box + gamepad + bottom button row, identical size/position/distribution in every game) materialized as a **shared component** — the ONLY things that vary per game are the display's content and the button mapping. **Asteroids is the ONLY reference** (display, gamepad, buttons — see `references/mobile-porter-todo.md`); caida/serpentina/bloque-buster get aligned to it, never the other way around. **Like `@skin-designer`, it DOES edit app code** — game components (`components/games/*`), game pages (`app/games/*/page.tsx`) and touch CSS in `app/globals.css` — but **never** the engines (`lib/games/*/game.ts`), the desktop layout (already consistent) or the skin system. Every change goes behind `isTouch` / `.av-player-touch` / coarse-pointer media queries. It records the unified sizing criterion in **`references/mobile-porter-todo.md`** (its living memory, same pattern as `game-skins.md`), keeps `npm run build` green, verifies with Playwright at 360×740 (screenshots to `.playwright-screenshots/`), and does **not** commit, push, or branch — the emulator isn't faithful, so the human validates on a real phone before committing. Inherited pending acceptance criterion from spec 11: 360×740 with no vertical scroll in all 4 games.
- **`@game-performance-booster`** — optimizes the **render/GC/loop performance** of a single game's engine (`lib/games/<id>/game.ts`) via **purely mechanical refactors that never change observable behavior** — same callback signals (`onScore`/`onLives`/`onLevel`/`onGameOver`), same physics/collisions/timing, same pixels on screen. **Like `@skin-designer`/`@mobile-porter`, it DOES edit app code**, but **only the engine** — never components, pages, CSS or the skin system, and never gameplay/rules/canvas size. It works on **one game at a time** (by `id`, passed explicitly) and never sweeps the catalog; it exists as a finishing step for **new** games via `/spec-impl-game`, not to retrofit the existing engines. It consults a fixed **known-patterns checklist** — P1 single-owner `requestAnimationFrame` (cancel before re-scheduling, the guard Caída's `init()` already has), P2 in-place array compaction over per-frame `filter()`/`concat()`, P3 hoisting per-entity per-frame `rgba(...)`/style construction into skin-time or alpha-bucketed caches, P4 caching static canvas content in a `Path2D`/offscreen snapshot, P5 pooling per-event `Audio` allocations — plus an explicit **anti-over-engineering list** it must NOT chase (no spatial partitioning, no dirty-rect clearing, no growth-capping — no engine ever exceeds ~100 live entities). It records each run in **`references/game-performance-booster-todo.md`** (its living memory, same pattern as `game-skins.md`), keeps `npm run build` green, and verifies "no behavior change" with a build + Playwright smoke (navigate to `/games/<id>`, zero new console errors, keys still respond). It does **not** commit — but in the `/spec-impl-game` chain the skill commits its diff into the game's PR after a green build + smoke (unlike `@mobile-porter`, whose diff stays uncommitted for real-device validation).
- **`@security-auditor`** — audits the platform's **security posture** across exactly **two domains**: the **Supabase database** (RLS per table, policy coverage per command, table GRANTs vs. what policies actually allow — note **RLS does not filter `TRUNCATE`** —, `SECURITY DEFINER` functions' `search_path` + ACL, extensions in `public`, `get_advisors`, and drift between the live remote and what the specs document) and the **app code** (input validation in Server Actions — every exported `'use server'` function is a public HTTP endpoint and TS types don't validate at runtime —, escaping user input in non-JSX output such as the contact email's HTML, secrets that could reach the client bundle, and response headers). **Like `@skin-designer`/`@mobile-porter`, it DOES edit app code** to fix what it finds, but **the database is strictly read-only**: it has no `apply_migration` tool, only `list_tables`/`execute_sql` (SELECT only)/`get_advisors`/`list_migrations`/`list_extensions`, and every DB remedy is handed over as a **proposed SQL block for the human to apply**. It works off a fixed checklist (**DB1–DB7** for the database, **AP1–AP5** for the app) with a severity scale (Crítico/Alto/Medio/Bajo/Informativo) that decides what gets fixed on the spot vs. only proposed, plus an explicit **anti-list** it must never re-raise: CSP, own rate limiting, requiring auth to save a score (all closed by `specs/13-security-hardening.md`), `npm audit`/dependency CVEs, route gating/OAuth (specs 12/14), and normal Supabase defaults (`relforcerowsecurity = false`, `postgres`/`service_role` bypassing RLS, the anon key being public by design, `using (true)` on a deliberately public SELECT policy, the `ensure_rls` trigger). It records every run in **`references/Security/audit-log.md`** (its living memory, same pattern as `game-skins.md`), which tracks **accepted risks** with a reopen condition so they're never re-reported as new; it **reads but never edits** `references/Security/checklist.md` (that's the spec 13 artifact). It keeps `npm run build` green, never prints a secret's value, and does **not** commit, push, branch, stash or revert — it snapshots `git status` first so it can tell its own diff apart from pre-existing uncommitted work. **Not to be confused with `/security-review`**, which reviews the _pending diff_ on a branch: `@security-auditor` monitors the _posture_ of the whole system (DB + code), with memory and repo-specific calibration.
